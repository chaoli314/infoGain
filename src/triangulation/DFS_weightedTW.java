...
public class DFS_weightedTW {
	...
	public Graph run(Graph G, int[] weights) {
		...
		long s_tts = utils.Helper.maxTableSize(s_cliques, weights);
		...
		best_tts = maxTableSize(best_cliques, weights); // upper bound;
		...
		for (int node = remaining.nextSetBit(0); node >= 0; node = remaining.nextSetBit(node + 1)) {
			...
			while (it.hasNext()) {
				...
				if (utils.Helper.isClique(clique, m_H)) {
					//m_tts += maxSize(clique, weights);
					m_cliques.add(clique);
				}
			}
			m_tts = maxTableSize(m_cliques, weights);
