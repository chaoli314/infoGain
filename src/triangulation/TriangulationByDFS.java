...
public class TriangulationByDFS {
	...
	private void expandNode(Graph graph, BitSet remaining, List<BitSet> cliques, long tts, int[] weights) {
		...
		for (int node = remaining.nextSetBit(0); node >= 0; node = remaining.nextSetBit(node + 1)) {
			...
			BitSet fa_U_G1 = new BitSet(m_H.V());
			eliminateVertex_DCM_2010(m_H, m_remaining, node, U, fa_U_G1);
			// remove
			...
			List<BitSet> newCliques = new BronKerboschCliqueFinder(m_H, fa_U_G1).getAllMaximalCliques();
	...
	public static void eliminateVertex_DCM_2010(Graph graph, BitSet missing, int node, BitSet fills, BitSet fa_U_G1) {
